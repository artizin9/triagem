...
export function CardAluno({aluno, Delete, UpdateInfo, Seetraining, type, SelectAluno, setSended}){
    ...
    const SelectAlunos = () => {
        ...
        if (select === "true") {
            setSended(sended => sended = true)
            console.log("teste")
        } else {
            setSended(sended => sended = false)
        }
    }
        ...
        <div className={`w-[90%] h-[60%] flex flex-col relative items-center rounded-lg p-2 shadow-md shadow-black/40 justify-between min-h-[320px] min-w-[200px] ${type === "aluno" ? 'min-h-[320px]' : 'min-h-[280px]'} ${type === "aluno" ? 'bg-primary-400' : 'bg-[#1a1919]'}`}>
            ...
            <div className="text-white flex flex-col w-full items-center justify-center text-center space-y-1">
                ...
                (<button 
                    ...
                    className={`w-full  whitespace-nowrap py-0.5 font-poppins font-bold shadow-black/30 shadow-md duration-500 ease-in-out cursor-pointer rounded-[3px] text-[14px] ${select ? 'bg-primary-100 text-white' : 'bg-white text-primary-400'}`}>
                        Selecionar Aluno
                    </button>)
                }
