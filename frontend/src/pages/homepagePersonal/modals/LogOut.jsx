const IconLogOut = (<svg className="w-[50%] aspect-square translate-x-7" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const ButtonClose = (<svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export function ModalLogOut({Close, Open, LogOut}){
    return (
        <div onClick={Close} className={`w-full h-full bg-black flex justify-center items-center bg-opacity-30 fixed insert-0 ${Open ? 'visible' : 'invisible'}`}>
            ...
            className={`bg-[#131313] flex flex-col items-center justify-center px-2 w-[30%] h-[55%] rounded-lg relative duration-300 ease-in-out ${Open ? 'scale-100 opacity-100' : 'scale-125 opacity-0'}`}>
                ...
                <div className="w-full items-center h-1/5 flex justify-center">
                    <button onClick={LogOut} className="flex items-center justify-center w-4/5 py-1 font-poppins font-semibold text-white bg-primary-100 text-[18px] rounded-lg">Sair</button>
                </div>
