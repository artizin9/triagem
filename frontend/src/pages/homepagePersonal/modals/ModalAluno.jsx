...
export function ModalAluno({aluno, setAluno, Close, Open}){
    ...
    function UpdateAluno(){
        ...
        setAluno((alunos) => 
            alunos.map((aluno) => aluno.id === id ? {...aluno, ...form} : aluno)
        )
        setMessageRight(true)
        setTimeout(() => {
            setMessageRight(false)
        ...
        <div onClick={Close} className={`w-full h-full bg-black flex justify-center items-center bg-opacity-30 fixed insert-0 ${Open ? 'visible' : 'invisible'}`}>
            <div 
                ...
                <div className="grid grid-cols-3 gap-6 w-[90%] gap-y-12 p-2 mt-6 place-items-center">
                    {FormFields.map((item, index) => (
                        <div key={index} className="w-full flex flex-col relative"> 
                            <label className="font-albert font-normal text-white">{item.label}</label>
                            {item.type === "select" ? (
                                <select
                                value={item.value}
                                ...
                                >
                                    <option className="text-white" value="">{item.name === "state" ? 'Selecione o estado' : 'Selecione a cidade'}</option>
                ...
                <div className="h-full w-full flex items-center justify-center relative">
                    ...
                    {aluno ? 'O aluno foi cadastrado com sucesso!' : 'Informações salvas com sucesso!'}</h1>
                    <button 
                    onClick={aluno ? CreateAluno : UpdateAluno}
                    className=" flex items-center justify-center mt-5 w-[30%] py-1 font-poppins font-semibold text-white bg-primary-100 shadow-md shadow-black/50  rounded-lg">{aluno ? 'Cadrastrar Aluno' : 'Salvar informações'}</button>
                </div>
