...
export function Main() {
    ...
    useEffect(() => {
        gsap.fromTo(title.current, 
            { y: '60%', opacity: 0 }, 
            { y: '0%', opacity: 1, duration: 1.2, ease: 'power4.out', delay: 1, stagger: 0.2 }
        )
    }, [])

    useEffect(() => {
        gsap.fromTo(subTitle.current, 
            { y: '60%', opacity: 0 }, 
            { y: '0%', opacity: 1, duration: 1.2, ease: 'power4.out', delay: 1.2, stagger: 0.2 }
        )
    }, [])

    useEffect(() => {
        gsap.fromTo(next.current, 
            { y: '40%', opacity: 0 }, 
            { y: '0%', opacity: 1, duration: 0.5, ease: 'power4.out', delay: 1.4, stagger: 0.2 }
        )
    }, [])
    ...
    useEffect(() => {
        gsap.fromTo(Header.current, 
            { y: '60%', opacity: 0 }, 
            { y: '0%', opacity: 1, duration: 1.2, ease: 'power4.out', delay: 0.5, stagger: 0.2 }
        )
    }, [])
    ...
    return (
        <div className="w-full h-[650px] items-center justify-center flex relative flex-col overflow-x-hidden">
            ...
            <div className="w-[95%] h-full pt-4 text-white flex flex-col">
                ...
                <div ref={Header} className="w-full h-[28%]">
                ...
                <div className="flex flex-col">
                    <h1 ref={title} className="font-poppins font-extrabold text-[35px] text-white">VOCÊ MERECE <span className="text-primary-100">RESULTADOS</span> <span className="block"> E AQUI ISSO <span className="text-primary-100">ACONTECE!</span> </span></h1>
                    <h2 ref={subTitle} className="font-albert font-medium text-[16px] mt-5">Não importa o seu ponto de partida, o que importa é a sua  <span className="block">vontade de mudar. Aqui, você encontra suporte e motivação </span> para alcançar a sua melhor versão.</h2>

                    <button ref={next} onClick={() => Navigate('/auth')} className="px-1 py-1.5 w-1/6 h-fit font-poppins font-extrabold text-white bg-primary-100 rounded-md mt-5 flex items-center justify-center space-x-5 duration-500 text-[17px] group">
                        <h1>CONTINUAR</h1>
